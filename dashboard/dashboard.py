...
if file_option == '选择储存至项目中的文件':
    ...
    if button_pd:
        ...
        if remark == "使用缓存数据" and has_cache:
            ...
            show_new_data(False, file_name, test_portion, src_threshold_value, False)
else:
    ...
    if file is None:
        ...
        src_threshold_value = handle_src_threshold_value(src_threshold_value)
        file_name = file.name
        has_cache, cache_text = is_has_cache('upload_' + file_name, test_portion, src_threshold_value)
        st.text(cache_text)
        if has_cache:
            remark = st.selectbox('数据更新（缓存）', ('使用缓存数据', '新建(更新)缓存数据（文件、比例或阈值变更）'))
        else:
        ...
        if button_pd:
            ...
            if remark == "使用缓存数据" and has_cache:
                ...
                show_new_data(False, file, test_portion, src_threshold_value, True)
